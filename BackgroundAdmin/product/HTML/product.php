...
$sql = "select shop_id from shop,user where shop.user_id=user.id";
//			根据搜索结果查询
...
<div class="information">
	...
	<div class="admininformation" style="background-color: rgb(245,245,245)">
		<div class="container">
			...
			//			根据搜索结果查询
			$sql = "select * from shop,user where shop.user_id=user.id";
			if (isset($_GET['searchtext']) && isset($_GET['sel'])) {
				if ($_GET['sel'] == 'shop_id') {
					$sql = $sql . " and {$_GET['sel']} = {$_GET['searchtext']} ";
				} else {
			...
			if ($result -> num_rows > 0) {
				while ($row = $result -> fetch_assoc()) {
					...
					<div class="col-lg-2 col-md-3 col-ms-3 col-xs-6">
						<div class="item">
							<span
								style="position: absolute;top: 0;color:#4d88ff;font-size: 16px"><?php echo $row['shop_id'] ?></span>
							...
							<p><a href="productitem.php?id=<?php echo $row['shop_id']; ?>" class="item_merchant"><font
										color="#4d88ff">●</font><?php echo $row['shop_text']; ?></a><br>
								<span class="item_merchant_place">店铺类型： <?php echo $row['shop_maintype']; ?></span><br>
								<span class="item_merchant_place">
									<a href="../../../HTML/user_other.php?id=<?php echo $row['id'] ?>" target="_blank">
									<img src="../../<?php echo $row['img_addr'] ?>" width="30" height="30"
