...
$sql = "update user set username='{$_POST['username']}',phone='{$_POST['phone']}',img_addr='{$true_img_name}',sex='{$_POST['sex']}',caree='{$_POST['caree']}',email='{$_POST['email']}',name='{$_POST['name']}',password='{$_POST['password']}' where id = {$_GET['id']}";
if ($conn -> query($sql)) {
	echo "<script>alert('修改商家成功！');location.assign('../HTML/usermerchant.php')</script>";
} else {
	echo "<script>alert('修改商家失败！');location.assign('../HTML/usermerchant.php')</script>";
}
?>
