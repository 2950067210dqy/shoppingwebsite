...
session_start();
require '../../conn.php';
$sql = "select id from user where email='{$_POST['email']}' and isadmin='merchant'";
$sql2 = "select id from user where username='{$_POST['username']}' and isadmin='merchant'";
$sql3 = "select id from user where phone='{$_POST['phone']}' and isadmin='merchant'";
$result = $conn -> query($sql);
$result2 = $conn -> query($sql2);
$result3 = $conn -> query($sql3);
if ($result -> num_rows && $result2 -> num_rows && $result3 -> num_rows) {
	echo "<script type='text/javascript'> alert('信息已被创建，请重新输入');  location.assign('../HTML/add_merchant.php');</script>";
}
//接受文件，临时文件信息
...
$sql = "insert into user values (null,'{$_POST['username']}','{$_POST['email']}','{$_POST['sex']}',
								...
								'{$true_img_name}','{$_POST['password']}','merchant',null)";
if ($conn -> query($sql)) {
	echo "<script>alert('添加商家成功！');location.assign('../HTML/usermerchant.php')</script>";
} else {
	echo "<script>alert('添加商家失败！');location.assign('../HTML/usermerchant.php')</script>";
}
?>
