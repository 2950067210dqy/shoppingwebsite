...
if (isset($_GET['sel']) && isset($_GET['searchtext']) && trim($_GET['searchtext']) == "") {
	echo "<script>alert('输入不能为空');location.assign('usermerchant.php')</script>";
}
...
$sql = "select id from user where isadmin='merchant'";
//			根据搜索结果查询
...
<div class="information">
	<div class="modal-title"
		...
		<span class="glyphicon glyphicon-th-large" aria-hidden="true"></span>所有商家 <span
			class="badge"><?php echo $RecordCount ?></span>
	</div>
	
	<div class="admininformation" style="background-color: rgb(245,245,245)">
		<form class="navbar-form navbar-left" action="add_merchant.php">
			<button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span> 添加商家</button>
		</form>
		<form class="navbar-form navbar-right" style="margin-right: 2%;" action="" method="get">
			<div class="text-center" STYLE="position: relative">
				...
				<select class="form-control" name="sel">
					...
					<option value="username">商家账号</option>
					<option value="email">商家邮箱</option>
					<option value="phone">商家电话</option>
					<option value="sex">商家性别</option>
					<option value="caree">商家职业</option>
					<option value="password">商家密码</option>
					<option value="invite_code">商家邀请码</option>
				</select>
				<button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-search"></span> 查询
				</button>
				<?php
				if (isset($_GET['searchtext']) && isset($_GET['sel'])) {
					echo "<a href='usermerchant.php' class='btn btn-default'>返回</a>";
				}
				?>
			</div>
		</form>
		...
		<table class="table table-hover table-striped  text-center">
			<form action="../PHP/delete_merchant.php" method="post">
				<tr>
					<td>
						序号
					</td>
				...
				//			根据搜索结果查询
				$sql = "select * from user where isadmin='merchant'";
				if (isset($_GET['searchtext']) && isset($_GET['sel'])) {
					if ($_GET['sel'] == 'id') {
						$sql = $sql . " and {$_GET['sel']} = {$_GET['searchtext']} ";
					} else {
					...
					while ($row = $result -> fetch_assoc()) {
						if ($row) {
							...
							<tr>
								...
								<td>
									<a href="update_merchant.php?id=<?php echo $row['id'] ?>"
									   class="btn btn-primary"><span class="glyphicon glyphicon-refresh"></span> 更新</a>
								</td>
								<td>
									<a href="../PHP/delete_merchant.php?id=<?php echo $row['id'] ?>"
