...
if (isset($_GET['sel']) && isset($_GET['searchtext']) && trim($_GET['searchtext']) == "") {
	echo "<script>alert('输入不能为空');location.assign('useradmin.php')</script>";
}
...
$sql = "select id from user where isadmin='true'";
//			根据搜索结果查询
...
<div class="information">
	<div class="modal-title"
		...
		<span class="glyphicon glyphicon-th" aria-hidden="true"></span>所有管理员 <span
			class="badge"><?php echo $RecordCount ?></span>
	</div>
	
	<div class="admininformation" style="background-color: rgb(245,245,245)">
		<form class="navbar-form navbar-left" action="add_admin.php">
			<button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span> 添加管理员</button>
		</form>
		<form class="navbar-form navbar-right" style="margin-right: 2%;" action="" method="get">
			<div class="text-center" STYLE="position: relative">
				...
				<?php
				if (isset($_GET['searchtext']) && isset($_GET['sel'])) {
					echo "<a href='useradmin.php' class='btn btn-default'>返回</a>";
				}
				?>
			</div>
		</form>
		...
		<table class="table table-hover table-striped  text-center">
			<form action="../PHP/delete_admin.php" method="post" id="form">
				<tr>
					<td>
						序号
					</td>
					...
					while ($row = $result -> fetch_assoc()) {
						if ($row) {
							...
							<tr>
								...
								<td>
									<a href="../PHP/delete_admin.php?id=<?php echo $row['id'] ?>"
									   class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span> 删除</a>
								</td>
								<td>
									<a href="../PHP/grant_admin.php?id=<?php echo $row['id'] ?>"
									   class="btn btn-success"><span class="glyphicon glyphicon-chevron-down"></span> 降权</a>
								</td>
								<td>
				...
				<tr>
					<Td colspan="13"></Td>
					<td>
						<button type="submit" id="jiangquan"
						        class="btn btn-success"><span class="glyphicon glyphicon-chevron-down"></span> 批量降权
						</button>
					</td>
					<td>
...
<script>
	$(function () {
		...
		$('#jiangquan').on('click', function () {
			$('#form').attr('action', '../PHP/grant_admin.php');
		});
		$('#deleteall').on('click', function () {
			$('#form').attr('action', '../PHP/delete_admin.php');
		});
	})
</script>
</body>
