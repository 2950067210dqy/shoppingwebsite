...
if (isset($_GET['sel']) && isset($_GET['searchtext']) && trim($_GET['searchtext']) == "") {
	echo "<script>alert('输入不能为空');location.assign('product_post_items.php?id={$_GET['id']}&shop_id={$shop_id}&type={$_GET['type']}')</script>";
}
?>
<!DOCTYPE html>
<html>
...
if (isset($_GET['id'])) {
	if ($_GET['sel'] == "content") {
		$sql = "select * from reply,user where user.id=reply.user_id  and replu_content like '%{$_GET['searchtext']}%' and diary_id in (select diary_id from diary where product_id = {$_GET['id']} and product_type='{$_GET['type']}')";
	} else {
		$sql = "select * from reply,user where user.id=reply.user_id  and {$_GET['sel']} like '%{$_GET['searchtext']}%' and diary_id in (select diary_id from diary where product_id = {$_GET['id']} and product_type='{$_GET['type']}')";
	}
	echo $sql;
...
$RecordCount = null;
if (isset($_GET['id'])) {
	$sql = "select diary_id from diary,user where user.id=diary.user_id and product_id = {$_GET['id']} and {$_GET['sel']} like '%{$_GET['searchtext']}%' and  product_type='{$_GET['type']}'";
	$result = $conn -> query($sql);
	...
	<div class="admininformation" style="background-color: rgb(245,245,245)">
		...
		<form class="navbar-form navbar-right" style="margin-right: 2%;" action="search_post.php" method="get">
			<div class="text-center" STYLE="position: relative">
				...
				<?php
				echo "<a href='product_post_items.php?id={$_GET['id']}&shop_id={$shop_id}&type={$_GET['type']}' class='btn btn-default'>返回</a>";
				?>
			</div>
		</form>
		...
		if ($_GET['id'] != "all") {
			...
			<table class="table table-hover table-striped  text-center">
				<?php
				echo " <tr><td colspan='8' class='text-center'>您想查询的是'{$_GET['searchtext']}',共查询到" . ($RecordCount + $RecordCounts) . "条记录 </td></tr>";
				?>
				<tr>
					<td>
						序号
		...
		<div class="shoppost">
			...
			<div id="postcontainer" class="container" style="width: 80%">
				...
				//		查找一级评论
				$sql = "select * from user,diary where user.id=diary.user_id and product_id=$id and {$_GET['sel']} like '%{$_GET['searchtext']}%' and  product_type='{$_GET['type']}' order by diary_id";
				$result = $conn -> query($sql);
...
";
								...
								if ($_GET['sel'] == "content") {
									$sql2 = " select * from user u  join reply r on r.diary_id={$row['diary_id']} and r.user_id=u.id and reply_content like '%{$_GET['searchtext']}%'  order by reply_id";
								} else {
									$sql2 = " select * from user u  join reply r on r.diary_id={$row['diary_id']} and r.user_id=u.id and {$_GET['sel']} like '%{$_GET['searchtext']}%'  order by reply_id";
								}
											...
											<div class="col-lg-8">
												<div class="row">
													...
													<div class="col-lg-8 text-left">
														<a href="<?php if (isset($_SESSION['id']) && $_SESSION['id'] == $row2['id']) echo '../../../HTML/user.php'; else echo "../../../HTML/user_other.php?id={$row2['id']}" ?>">
															<?php
															//					检测是否为管理员
														...
														if ($row2['last_id'] != 0) {
															if ($_GET['sel'] == "content") {
																$sql3 = "select * from user where reply_content like '%{$_GET['searchtext']}%' and id = (select user_id from reply where reply_id={$row2['last_id']}) ";
															} else {
																$sql3 = "select * from user where {$_GET['sel']} like '%{$_GET['searchtext']}%' and id = (select user_id from reply where reply_id={$row2['last_id']}) ";
															}
...
//										$postinid ++;
	...
	<!--							折叠多级评论-->
			...
			<!--		必须登录才会显示主评论框，否则提示登录-->
			<?php
			if (isset($_SESSION['id']) && ($RecordCounts + $RecordCount) > 0) {
				?>
