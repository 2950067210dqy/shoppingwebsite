...
session_start();
require '../../conn.php';
$sql = "select shop_id from shop where shop_name ='{$_POST['shop_name']}'";
echo "<script>console.log($sql);</script>";
$result = $conn -> query($sql);
if ($result -> num_rows) {
	echo "<script type='text/javascript'> alert('店铺名已被创建，请重新输入');  location.assign('../HTML/add_shop.php');</script>";
}
//接受文件，临时文件信息

$fileinfo = $_FILES["shop_img"];//降维操作
...
if ($error == 0) {
	...
	$true_img_name = $_POST['beforeheadimg'];
}
$sql = "insert into shop values (null,{$_SESSION['id']},'{$_POST['shop_name']}','{$true_img_name}','{$_POST['shop_text']}','{$_POST['shop_maintype']}',null)";
if ($conn -> query($sql)) {
	echo "<script>alert('添加店铺成功！');location.assign('../HTML/shop.php')</script>";
} else {
	echo "<script>alert('添加店铺失败！');location.assign('../HTML/shop.php')</script>";
}
?>
