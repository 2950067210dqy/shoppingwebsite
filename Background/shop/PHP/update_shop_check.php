...
$sql = "update shop set shop_name='{$_POST['shop_name']}',shop_text='{$_POST['shop_text']}',shop_img_addr='{$true_img_name}',shop_maintype='{$_POST['shop_maintype']}' where shop_id = {$_GET['id']}";
if ($conn -> query($sql)) {
	echo "<script>alert('修改店铺成功！');location.assign('../HTML/shop.php')</script>";
} else {
	echo "<script>alert('修改店铺失败！');location.assign('../HTML/shop.php')</script>";
}
?>
