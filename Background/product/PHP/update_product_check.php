...
if ($error == 0) {
	...
	$true_img_name = $_POST['beforeimg'];
}
$sql = "update products set title='{$_POST['title']}',price={$_POST['price']},img_addre='{$true_img_name}',type='{$_POST['type']}' where id = {$_POST['id']}";
if ($conn -> query($sql)) {
	
	echo "<script>alert('修改商品成功！');location.assign('../HTML/productitem.php?id={$_POST['shop_id']}')</script>";
} else {
	echo "<script>alert('修改商品失败！');location.assign('../HTML/productitem.php?id={$_POST['shop_id']}')</script>";
}
?>
