		...
		<table class="table table-hover table-striped  text-center">
			<form action="../PHP/delete_product.php" method="post">
				...
				if (isset($_GET['searchtext']) && isset($_GET['sel'])) {
					...
					echo " <tr><td colspan='10' class='text-center'>您想查询的是'{$_GET['searchtext']}',共查询到{$RecordCount}条记录 </td></tr>";
				}
				...
				if ($result -> num_rows > 0) {
								...
								<td>
									<a href="../../../HTML/shop.php?id=<?php echo $row['shop_id'] ?>"
									   target="_blank"><img src="../../<?php echo $row['shop_img_addr'] ?>" width="86"
									                        height="86" style="border-radius: 10%"></a></a>
								</td>
								<td>
									<?php echo $row['time'] ?>
								</td>
								<td>
									<a href="update_product.php?id=<?php echo $row['shop_id'] ?>"
					...
					if (isset($_GET['searchtext']) && isset($_GET['sel'])) {
						echo "<tr><td colspan='10' class='text-center'>暂无结果，请换个关键词查询把！<td><tr>";
					} else {
						echo "<tr><td colspan='10' class='text-center'>暂无商品，快去添加吧！<td><tr>";
					}
				}
				$result -> free_result();
				?>
