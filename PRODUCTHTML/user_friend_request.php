...
//	在发一条请求消息请求类型（type）为send_true给加好友的用户，提示该用户已经同意加好友
	$sql = "insert into add_user_message values (null,{$user_id},{$send_user_id},'send_true',null,null,null)";
	$conn -> query($sql);
...
//	将当前的好友请求消息的请求类型（type）置为add_false
	...
	$sql = "insert into add_user_message values (null,{$user_id},{$send_user_id},'send_false',null,null,null)";
	$conn -> query($sql);
}
?>
